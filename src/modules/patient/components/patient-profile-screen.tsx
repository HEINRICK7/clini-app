...
export function PatientProfileScreen({ patientId }: { patientId: string }) {
  ...
  return <section className="mx-auto grid w-full max-w-2xl gap-4">
    ...
    {tab === "history" ? <div className="grid gap-3"><HistoryTab isPending={evolutionsQuery.isPending} items={evolutionsQuery.data?.items ?? []} /><CliniOdontogram patientId={patient.id} readOnly unitId={patient.currentUnitId} /></div> : null}
    {tab === "treatments" ? <div className="grid gap-3"><TreatmentsTab isPending={treatmentsQuery.isPending} items={treatmentsQuery.data?.items ?? []} /><CliniOdontogram patientId={patient.id} readOnly unitId={patient.currentUnitId} /></div> : null}
...
function SummaryTab({ patient, nextAppointment, activeTreatment, onEdit }: { patient: Patient; nextAppointment?: { startsAt: string; endsAt: string; type: string }; activeTreatment?: { name: string; status: string; plannedProcedures: { name: string }[] }; onEdit: () => void }) {
  return <div className="grid gap-3"><Card className="p-5"><div className="flex items-center gap-2 text-primary"><CalendarPlus aria-hidden="true" className="h-4 w-4" /><p className="text-xs font-bold uppercase tracking-[0.14em]">Próxima consulta</p></div>{nextAppointment ? <><p className="mt-3 text-lg font-bold text-brand-navy">{new Date(nextAppointment.startsAt).toLocaleDateString("pt-BR")} · {new Date(nextAppointment.startsAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p><p className="mt-1 text-sm text-muted-foreground">{appointmentTypeLabel(nextAppointment.type)}</p></> : <p className="mt-3 text-sm text-muted-foreground">Nenhuma consulta futura encontrada.</p>}</Card><Card className="p-5"><div className="flex items-center gap-2 text-primary"><FileClock aria-hidden="true" className="h-4 w-4" /><p className="text-xs font-bold uppercase tracking-[0.14em]">Tratamento atual</p></div>{activeTreatment ? <><p className="mt-3 font-bold text-brand-navy">{activeTreatment.name}</p><p className="mt-1 text-sm text-muted-foreground">{activeTreatment.plannedProcedures.length} procedimento(s) planejado(s) · {activeTreatment.status === "ACTIVE" ? "Em andamento" : activeTreatment.status}</p></> : <p className="mt-3 text-sm text-muted-foreground">Nenhum tratamento ativo.</p>}</Card><Card className="p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-warning">Dados do paciente</p><p className="mt-2 text-sm text-muted-foreground">{patient.email ?? "Sem e-mail"} · {patient.address ?? "Sem endereço"}</p></div><Button onClick={onEdit} size="sm" variant="outline">Editar</Button></div></Card><CliniOdontogram patientId={patient.id} readOnly unitId={patient.currentUnitId} /></div>;
}
