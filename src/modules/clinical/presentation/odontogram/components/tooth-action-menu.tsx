...
export function ToothActionMenu({ action, onActionChange, procedures, proceduresLoading, treatments, treatmentsLoading, patientId, input, onInputChange, onSubmit, disabled }: {
  ...
  return <div className="grid gap-4">
    ...
    {action ? <div className="rounded-2xl border border-primary/20 bg-blue-50/50 p-3">
      <div className="mb-3 flex items-center justify-between gap-3"><p className="text-sm font-bold text-primary">Novo registro · {actionLabels[action].label}</p><span className="text-xs text-muted-foreground">Você pode escolher outra ação acima</span></div>
      {action === "PROCEDURE" ? <RelatedSelect disabled={disabled} emptyDescription="Cadastre o procedimento no catálogo para poder registrá-lo no dente." emptyHref="/more?section=catalog" emptyLabel="Cadastrar procedimento" label="Procedimento realizado" loading={proceduresLoading} onChange={(value) => onInputChange({ ...input, procedureId: value })} options={procedures.map((procedure) => ({ value: procedure.id, label: procedure.name }))} value={input.procedureId ?? ""} /> : null}
      {action === "PLANNING" ? <RelatedSelect disabled={disabled} emptyDescription="Crie um tratamento e adicione um procedimento planejado antes de vinculá-lo ao dente." emptyHref={`/more?section=treatments&patientId=${encodeURIComponent(patientId)}`} emptyLabel="Criar planejamento" label="Item do tratamento" loading={treatmentsLoading} onChange={(value) => onInputChange({ ...input, planItemId: value })} options={planned.map(({ treatment, item }) => ({ value: item.id, label: `${treatment.name} · ${item.name}` }))} value={input.planItemId ?? ""} /> : null}
      {action !== "NOTE" ? <label className="mt-3 grid gap-1.5 text-sm font-semibold"><span>Status</span><select className="min-h-11 rounded-xl border border-border bg-surface px-3 font-normal" disabled={disabled} onChange={(event) => onInputChange({ ...input, status: (event.target.value || undefined) as ToothRecordStatus | undefined })} value={input.status ?? ""}><option value="">{action === "PROCEDURE" ? "Concluído" : "Aberto"}</option><option value="OPEN">Aberto</option><option value="IN_PROGRESS">Em andamento</option><option value="COMPLETED">Concluído</option><option value="CANCELED">Cancelado</option></select></label> : null}
      <label className="mt-3 grid gap-1.5 text-sm font-semibold"><span>{action === "NOTE" ? "Observação clínica" : "Descrição (opcional)"}</span><textarea className="min-h-20 rounded-xl border border-border bg-surface px-3 py-2 text-sm font-normal" disabled={disabled} maxLength={2000} onChange={(event) => onInputChange({ ...input, description: event.target.value })} placeholder={action === "NOTE" ? "Escreva uma observação para este dente" : "Descreva o registro clínico"} value={input.description} /></label>
