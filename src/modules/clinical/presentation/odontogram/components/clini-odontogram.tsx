...
export function CliniOdontogram({ patientId, unitId, appointmentId, readOnly = false }: { patientId: string; unitId: string; appointmentId?: string; readOnly?: boolean }) {
  ...
  return <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(20rem,0.45fr)]">
    <Card className="min-w-0 overflow-hidden p-4 sm:p-6">
      ...
      <div className="mt-5 overflow-x-auto rounded-2xl border border-border bg-surface-muted p-3 sm:p-5"><div className="mx-auto min-w-[520px] max-w-3xl"><Odontogram key={selectedTeeth.join(",")} defaultSelected={selectedTeeth.map((id) => `teeth-${id}`)} onChange={handleChange} readOnly={false} showLabels showTooltip={false} notation="FDI" layout="square" teethConditions={conditionGroups} colors={{ darkBlue: "#102a43", baseBlue: "#2563eb", lightBlue: "#dbeafe" }} /></div></div>
