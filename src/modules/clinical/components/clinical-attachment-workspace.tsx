...
import type { ClinicalAttachment } from "@/app/services";
...
export function ClinicalAttachmentWorkspace() {
  const { clinicalAttachments, clinicalDocuments, patient: patientService, practice } = useCliniServices();
  ...
  const patientsQuery = useQuery({ queryKey: ["patients", "clinical-attachments"], queryFn: () => patientService.listPatients(), retry: false });
  ...
  const attachmentsQuery = useQuery({ queryKey: ["clinical-attachments", patientId], queryFn: () => clinicalAttachments.listClinicalAttachments(patientId), enabled: Boolean(patientId), retry: false });
  ...
  const uploadMutation = useMutation({ mutationFn: () => clinicalAttachments.uploadClinicalAttachment({ patientId, unitId: patient?.currentUnitId ?? "", clinicalDocumentId: documentId || undefined, file: file! }), onSuccess: async (saved) => { setFile(null); if (inputRef.current) inputRef.current.value = ""; setMessage(`Anexo ${saved.originalFilename} armazenado com checksum.`); await refresh(); }, onError: showError });
  const archiveMutation = useMutation({ mutationFn: clinicalAttachments.archiveClinicalAttachment, onSuccess: async () => { setMessage("Anexo arquivado sem apagar o conteúdo."); await refresh(); }, onError: showError });
  ...
  function showError(error: unknown) { setMessage(apiErrorMessage(error, "Não foi possível concluir a operação do anexo.")); }
