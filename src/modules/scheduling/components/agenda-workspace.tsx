  ...
  return (
    <section className="grid min-w-0 gap-5">
      <div className="flex min-w-0 items-start justify-between gap-3">
        <div className="min-w-0"><p className="text-sm font-semibold text-primary">Sua rotina</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-brand-navy">Agenda</h1><p className="mt-1 break-words text-sm text-muted-foreground">Organize os atendimentos do seu dia.</p></div>
...
function AgendaField({ label, type = "text", value, onChange }: { label: string; type?: string; value: string; onChange: (value: string) => void }) {
  return <label className="grid min-w-0 gap-1.5 text-sm font-semibold"><span>{label}</span><input className="min-h-12 min-w-0 w-full rounded-xl border border-border bg-surface px-4 text-base font-normal outline-none focus:border-primary focus:ring-4 focus:ring-cyan-100" onChange={(event) => onChange(event.target.value)} type={type} value={value} /></label>;
}
